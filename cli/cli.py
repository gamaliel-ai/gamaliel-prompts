...
Examples:
  ...
  %(prog)s validate
    ...
    chat_parser.add_argument("--template", "-t", default="chat_agent", help="Template to use")
    chat_parser.add_argument("--book", "-b", help="Focus on specific Bible book")
    chat_parser.add_argument("--chapter", "-c", type=int, help="Focus on specific chapter")
    ...
    test_parser.add_argument("--render-only", action="store_true", help="Only render template, don't call LLM")
    
    # Scripture command
    scripture_parser = subparsers.add_parser("scripture", help="Scripture operations")
    scripture_subparsers = scripture_parser.add_subparsers(dest="scripture_command", help="Scripture subcommands")
    
    # Scripture get command
    get_parser = scripture_subparsers.add_parser("get", help="Get specific verse/chapter")
    get_parser.add_argument("reference", help="Scripture reference (e.g., 'John 3:16' or 'Genesis 1')")
    ...
    index_parser = scripture_subparsers.add_parser("index", help="Build/rebuild search index")
    
    # Validate command
    validate_parser = subparsers.add_parser("validate", help="Validate prompt templates and configs")
    validate_parser.add_argument("target", nargs="?", default=".", help="File or directory to validate")
    validate_parser.add_argument("--templates-only", action="store_true", help="Only validate template syntax")
    validate_parser.add_argument("--profiles-only", action="store_true", help="Only validate profile files")
    validate_parser.add_argument("--all", action="store_true", help="Validate all components (default)")
    validate_parser.add_argument("--fix", action="store_true", help="Automatically fix violations using AI")
...
def handle_scripture_search(args: argparse.Namespace) -> int:
    ...
    result = execute_tool("search_scripture_semantic", query=args.query, n_results=args.max_results)
...
def handle_scripture_index() -> int:
    ...
    if success:
        ...
        print("Available books:", ", ".join(books[:10]) + ("..." if len(books) > 10 else ""))
        return 0
    else:
